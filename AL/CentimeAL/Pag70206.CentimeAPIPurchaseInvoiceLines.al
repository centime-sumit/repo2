...
page 70206 "Centime API - Purchase Invoice"
{
    ...
    APIVersion = 'v2.0';
    ApplicationArea = All;
    Caption = 'centimeAPIPurchaseInvoice';
    DelayedInsert = true;
    ...
    layout
    {
        area(content)
        {
            repeater(Group)
            {
                ...
                {
                    Caption = 'Derived G/L Account No.';
                    Editable = false;
                }

                field(derivedGLAccountName; GetResolvedGLAccountName())
                {
                    Caption = 'Derived G/L Account Name';
                    Editable = false;
                }
    ...
    local procedure ResolveLineGL(LineAggRec: Record "Purch. Inv. Line Aggregate"; var GLNo: Code[20]; var GLName: Text[100])
    var
        PurchInvHeader: Record "Purch. Inv. Header";
        PurchInvLine: Record "Purch. Inv. Line";
        PurchInvEntityAgg: Record "Purch. Inv. Entity Aggregate";
        TempPurchLine: Record "Purchase Line";
        PurchLineFound: Boolean;
        hdrFound: Boolean;
        apiTypeText: Text[50];
        debugTextFull: Text[250];
    begin
        GLNo := '';
        GLName := '';
        ...
        begin
            // --- Resolve Purch. Inv. Entity Aggregate using Id (SystemId) ---
            PurchInvEntityAgg.Reset();
            PurchInvEntityAgg.SetRange(Id, LineAggRec."Document Id");

            if not PurchInvEntityAgg.FindFirst() then begin
                GLNo := 'ENTITY-NOT-FOUND';
                GLName := 'NO-ENTITY-FOR-DOCID';
                exit;
            end;

            // --- Ensure invoice is posted ---
            if PurchInvEntityAgg."No." = '' then begin
                GLNo := 'NOT-POSTED';
                GLName := 'ENTITY-NO-NO';
                exit;
            end;

            // --- Resolve posted header using document No. ---
            if not PurchInvHeader.Get(PurchInvEntityAgg."No.") then begin
                GLNo := 'HEADER-NOT-FOUND';
                GLName := 'NO-HDR-FOR-NO';
                exit;
            end;
        ...
        // --- 2️⃣ If header found, find the posted line by Line No. ---
        if hdrFound then begin
            PurchInvLine.SetRange("Document No.", PurchInvHeader."No.");
            PurchInvLine.SetRange("Line No.", LineAggRec."Line No.");
            if PurchInvLine.FindFirst() then
                PurchLineFound := true;
        end;

        // --- 3️⃣ If posted line found, build temporary Purchase Line for resolver ---
        if PurchLineFound then begin
            ...
            TempPurchLine."Buy-from Vendor No." := PurchInvLine."Buy-from Vendor No.";
            TempPurchLine."Location Code" := PurchInvLine."Location Code";
            TempPurchLine."Document No." := PurchInvHeader."No.";
