...
codeunit 70220 "CustomerGenJournalApply"
{
    ...
    var
        TempApplyingCustomerLedgerEntry: Record "Cust. Ledger Entry" temporary;
    ...
    begin
        ...
        TempApplyingCustomerLedgerEntry."Posting Date" := GenJournalLine."Posting Date";
        TempApplyingCustomerLedgerEntry."Document Type" := GenJournalLine."Document Type";
        TempApplyingCustomerLedgerEntry."Document No." := GenJournalLine."Document No.";
        if GenJournalLine."Bal. Account Type" = GenJournalLine."Bal. Account Type"::Customer then begin
            TempApplyingCustomerLedgerEntry."Customer No." := GenJournalLine."Bal. Account No.";
            Customer.Get(TempApplyingCustomerLedgerEntry."Customer No.");
            TempApplyingCustomerLedgerEntry.Description := Customer.Name;
        end else begin
            TempApplyingCustomerLedgerEntry."Customer No." := GenJournalLine."Account No.";
            TempApplyingCustomerLedgerEntry.Description := GenJournalLine.Description;
        end;
        TempApplyingCustomerLedgerEntry."Currency Code" := GenJournalLine."Currency Code";
        TempApplyingCustomerLedgerEntry.Amount := GenJournalLine.Amount;
        TempApplyingCustomerLedgerEntry."Remaining Amount" := GenJournalLine.Amount;
        if TempApplyingCustomerLedgerEntry."Posting Date" < CustomerLedgerEntry."Posting Date" then
            Error(
                EarlierPostingDateErr, TempApplyingCustomerLedgerEntry."Document Type", TempApplyingCustomerLedgerEntry."Document No.",
                CustomerLedgerEntry."Document Type", CustomerLedgerEntry."Document No.");

        if TempApplyingCustomerLedgerEntry."Entry No." <> 0 then begin
            OnSetCustApplIdAPIOnBeforeCheckAgainstApplnCurrency(GenJournalLine, CustomerLedgerEntry);
            GenJnlApply.CheckAgainstApplnCurrency(
                ApplnCurrencyCode, CustomerLedgerEntry."Currency Code", GenJournalLine."Account Type"::Customer, true);
        end;

        CustomerLedgerEntry.SetRange("Entry No.", CustomerLedgerEntry."Entry No.");
        CustomerLedgerEntry.SetRange("Customer No.", CustomerLedgerEntry."Customer No.");
        CustEntrySetApplID.SetApplId(CustomerLedgerEntry, TempApplyingCustomerLedgerEntry, GenJournalLine."Applies-to ID");
    end;
    ...
    procedure ApplyCustomerLedgerEntryAPI(var GenJournalLine: Record "Gen. Journal Line")
    var
        CustomerLedgerEntry: Record "Cust. Ledger Entry";
        TempCustomerLedgerEntry: Record "Cust. Ledger Entry" temporary;
    begin
        GenJnlLine.Copy(GenJournalLine);
        GetCurrency();
        GenJnlLine.GetAccTypeAndNo(GenJnlLine, AccType, AccNo);
        GetAppliedCustomerEntries(TempCustomerLedgerEntry, GenJnlLine);
        GenJnlLine."Applies-to ID" := GenJnlLine."Document No.";
        CustomerLedgerEntry.Reset();
        CustomerLedgerEntry.SetCurrentKey("Customer No.", Open, Positive);
        CustomerLedgerEntry.SetRange("Customer No.", AccNo);
        ...
        if CustomerLedgerEntry.Find('-') then begin
            CurrencyCode2 := CustomerLedgerEntry."Currency Code";
            if GenJnlLine.Amount = 0 then begin
                repeat
                    if not TempCustomerLedgerEntry.Get(CustomerLedgerEntry."Entry No.") then begin
                        PaymentToleranceMgt.DelPmtTolApllnDocNo(GenJnlLine, CustomerLedgerEntry."Document No.");
                        OnApplyCustomerLedgerEntryAPIOnBeforeCheckAgainstApplnCurrencyAmountZero(GenJnlLine, CustomerLedgerEntry);
                        CheckAgainstApplnCurrency(CurrencyCode2, CustomerLedgerEntry."Currency Code", AccType::Customer, true);
                        UpdateCustLedgEntry(CustomerLedgerEntry);

                        if PaymentToleranceMgt.CheckCalcPmtDiscGenJnlCust(GenJnlLine, CustomerLedgerEntry, 0, false) and
                           (Abs(CustomerLedgerEntry."Amount to Apply") >=
                            Abs(CustomerLedgerEntry."Remaining Amount" - CustomerLedgerEntry.GetRemainingPmtDiscPossible(GenJnlLine."Posting Date")))
                        then
                            GenJnlLine.Amount := GenJnlLine.Amount - (CustomerLedgerEntry."Amount to Apply" - CustomerLedgerEntry."Remaining Pmt. Disc. Possible")
                        else
                            GenJnlLine.Amount := GenJnlLine.Amount - CustomerLedgerEntry."Amount to Apply";
                    end;
                until CustomerLedgerEntry.Next() = 0;
                TempCustomerLedgerEntry.DeleteAll();
                if (GenJnlLine."Bal. Account Type" = GenJnlLine."Bal. Account Type"::Customer) or (GenJnlLine."Bal. Account Type" = GenJnlLine."Bal. Account Type"::Vendor) then
                    GenJnlLine.Amount := -GenJnlLine.Amount;
                GenJnlLine.Validate(Amount);
            end else
                repeat
                    OnApplyCustomerLedgerEntryAPIOnBeforeCheckAgainstApplnCurrencyAmountNonZero(GenJnlLine, CustomerLedgerEntry);
                    CheckAgainstApplnCurrency(CurrencyCode2, CustomerLedgerEntry."Currency Code", AccType::Customer, true);
                until CustomerLedgerEntry.Next() = 0;
            if GenJnlLine."Currency Code" <> CurrencyCode2 then
                if GenJnlLine.Amount = 0 then
                    GenJnlLine."Currency Code" := CustomerLedgerEntry."Currency Code"
                else begin
                    OnApplyCustomerLedgerEntryAPIOnBeforeCheckAgainstApplnCurrencyDifferentCurrenciesAmountNonZero(GenJnlLine, CustomerLedgerEntry);
                    CheckAgainstApplnCurrency(GenJnlLine."Currency Code", CustomerLedgerEntry."Currency Code", AccType::Customer, true);
                end;
            GenJnlLine."Applies-to Doc. Type" := GenJnlLine."Applies-to Doc. Type"::" ";
            GenJnlLine."Applies-to Doc. No." := '';
            OnApplyCustomerLedgerEntryOnAfterSetCustomerAppliesToDocNo(GenJnlLine, CustomerLedgerEntry);
        end else
            GenJnlLine."Applies-to ID" := '';
    ...
    [IntegrationEvent(false, false)]
    local procedure OnSetCustApplIdAPIOnBeforeCheckAgainstApplnCurrency(GenJournalLine: Record "Gen. Journal Line"; CustomerLedgerEntry: Record "Cust. Ledger Entry")
    begin
    end;
    ...
    [IntegrationEvent(false, false)]
    local procedure OnApplyCustomerLedgerEntryAPIOnBeforeCheckAgainstApplnCurrencyAmountZero(GenJournalLine: Record "Gen. Journal Line"; CustomerLedgerEntry: Record "Cust. Ledger Entry")
    begin
    end;
    ...
    [IntegrationEvent(false, false)]
    local procedure OnApplyCustomerLedgerEntryAPIOnBeforeCheckAgainstApplnCurrencyAmountNonZero(GenJournalLine: Record "Gen. Journal Line"; CustomerLedgerEntry: Record "Cust. Ledger Entry")
    begin
    end;
    ...
    [IntegrationEvent(false, false)]
    local procedure OnApplyCustomerLedgerEntryAPIOnBeforeCheckAgainstApplnCurrencyDifferentCurrenciesAmountNonZero(GenJournalLine: Record "Gen. Journal Line"; CustomerLedgerEntry: Record "Cust. Ledger Entry")
    begin
    end;
