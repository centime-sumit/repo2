...
page 70230 "Purchase Orders"
{
    ...
    layout
    {
        area(Content)
        {
            repeater(Group)
            {
                ...
                field(CreatedByVar; CreatedByVar)
                {
                    Caption = 'CreatedByVar';
                }
                field(DueDate; DueDate)
                {
                    Caption = 'DueDate';
                }
                field(VendorInvoiceNumber; VendorInvoiceNumber)
                {
                    Caption = 'VendorInvoiceNumber';

                    trigger OnValidate()
                    begin
                        VendorInvoiceNumberSet := true;
                    end;
                }
                ...
                field(CreatedByGuid; CreatedByGuid)
                {
                    Caption = 'CreatedByGuid';
                }
                ...
                part(vendorPostingGroups; "VendorPostingGroups")
                {
                    Caption = 'Vendor Posting group';
                    EntityName = 'vendorPostingGroup';
                    EntitySetName = 'vendorPostingGroups';
                    Multiplicity = ZeroOrOne;
                    SubPageLink = "Code" = field("Vendor Posting Group");
                }
            }
        }
    }
    ...
    var
        ...
        DueDate: Date;
        VendorInvoiceNumber: Code[35];
        VendorInvoiceNumberSet: Boolean;
    ...
    local procedure ReceiveOrder(var PurchaseHeader: Record "Purchase Header")
    var
        LinesInstructionMgt: Codeunit "Lines Instruction Mgt.";
    begin
        LinesInstructionMgt.PurchaseCheckAllLinesHaveQuantityAssigned(PurchaseHeader);
    ...
    local procedure getFromPurchaseHeader()
    var
        PurchaseHeader: Record "Purchase Header";
    begin
        ...
        if PurchaseHeader.FindSet() then begin
            ...
            VendorInvoiceNumber := PurchaseHeader."Vendor Invoice No.";
            ExchangeRate := PurchaseHeader."Currency Factor";
            VendorPostingGroup := PurchaseHeader."Vendor Posting Group";
            PaymentTermsCode := PurchaseHeader."Payment Terms Code";
            CreatedByGuid := PurchaseHeader.SystemCreatedBy;
        end;
    end;

    local procedure UpdateVendorInvoiceNo()
    var
        PurchaseHeader: Record "Purchase Header";
    begin
        if not VendorInvoiceNumberSet then
            exit;

        if not PurchaseHeader.Get(PurchaseHeader."Document Type"::Order, Rec."No.") then
            exit;

        PurchaseHeader."Vendor Invoice No." := VendorInvoiceNumber;
        PurchaseHeader.Modify(false);
    end;

    local procedure GetCreatedByCode()
    var
        User: Record User;
    begin
        User.SetRange("User Security ID", Rec.SystemCreatedBy);
        if (User.FindSet()) then begin
            CreatedByVar := User."User Name";
        end;
    end;
